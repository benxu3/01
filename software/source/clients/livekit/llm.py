...
from livekit.plugins.openai.models import ChatModels
from livekit.plugins.openai.utils import get_base_url
...
class OpenLLM(llm.LLM):
    ...
    async def chat(
        ...
        print("messges from openai ctx: ", messages)
        ...
        print("completion from chat completions: ", cmp)

        return OpenLLMStream(cmp, fnc_ctx)
...
class OpenLLMStream(llm.LLMStream):
    def __init__(
    ...
    def __aiter__(self) -> "OpenLLMStream":
        return self
    ...
    async def __anext__(self) -> llm.ChatChunk:
        ...
        async for chunk in self._oai_stream:
            print("chunk in openai async stream: ", chunk)
            for i, choice in enumerate(chunk.choices):
                delta = choice.delta
                print("delta, i, choice is: ", delta, i, choice)
